...
# it will be deleted using the Retentionmonthly value and another Lambda function 
...
def lambda_handler(event, context):
    ...
    reservations = ec.describe_instances(
        Filters=[
            {'Name': 'tag-key', 'Values': ['BackupMonthly']},
        ...
        try:
            ...
            try:
               ...
               # if(ec.describe_instance_status(InstanceIds=[instance['InstanceId']],Filters=[{ 'Name': 'instance-state-name','Values': ['running'] }])['InstanceStatuses'][0]['InstanceState']['Name'] == 'running'):    
                ...
                AMIid = ec.create_image(InstanceId=instance['InstanceId'], Name="Lambda -Monthly " + [result['Value'] for result in instance['Tags'] if result['Key'] == 'Name'][0] + " - " + " From " + create_fmt, Description="Lambda created AMI of instance " + instance['InstanceId'], NoReboot=True, DryRun=False)
